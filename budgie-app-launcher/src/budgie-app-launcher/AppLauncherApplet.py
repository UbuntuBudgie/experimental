...
class AppLauncherApplet(Budgie.Applet):
    ...
    def buildIndicator(self):
        ...
        self.indicatorIcon = Gtk.Image.new_from_icon_name("budgie-app-launcher-applet-symbolic", Gtk.IconSize.MENU)
    ...
    def buildStackPage1(self):
        ...
        self.contentScroll.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
    ...
    def buildStackPage2(self):
        ...
        self.allAppsContainer.pack_start(self.allAppsContentScroll, True, True, 0)
        self.allAppsContentScroll.set_overlay_scrolling(True)
        self.allAppsContentScroll.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
        ...
        page2BottomBar.pack_start(page2BottomBarShowPanelContainer ,False, False, 0)
        showOnPanelLabel = Gtk.Label("Show on panel", xalign=0)
        page2BottomBarShowPanelContainer.pack_start(showOnPanelLabel, True, True, 0)
        showOnPanelLabel.get_style_context().add_class("dim-label")
        self.showOnPanelSpinButton = Gtk.SpinButton()
        self.showOnPanelSpinButton.set_adjustment(Gtk.Adjustment(0, 0, 900, 1, 10, 0))
        self.showOnPanelSpinButton.set_value(self.showOnPanel)
        page2BottomBarShowPanelContainer.pack_start(self.showOnPanelSpinButton, False, False, 0)
        self.showOnPanelSpinButton.connect("value-changed", self.showOnPanelSpinButtonOnValueChange)
        ## page2BottomBar icon size Container
        page2BottomBarIconSizeContainer = Gtk.HBox()
        page2BottomBar.pack_start(page2BottomBarIconSizeContainer ,False, False, 0)
        iconSizeLabel = Gtk.Label("Icon size", xalign=0)
        page2BottomBarIconSizeContainer.pack_start(iconSizeLabel, True, True, 0)
        ...
        page2BottomBarIconSizeContainer.pack_start(iconSizeSpinButton, False, False, 0)
        iconSizeSpinButton.connect("value-changed", self.iconSizeSpinButtonOnValueChange)
    ####################################
    # build END
    ####################################
    ...
    def upButtonOnClick(self, button):
        if self.currentMenuButton in self.menuButtons:
            ...
            if newIndex >= 0:
                self.menuButtonsContainer.reorder_child(self.currentMenuButton, newIndex)
    ...
    def doneButtonOnClick(self, button):
        ...
        self.doneButton.set_sensitive(False)
        self.showOnPanelSpinButton.get_adjustment().set_upper(len(self.activeApps))
    ...
    def menuButtonOnToggle(self, toggleButton, *data):
        ...
        if self.currentMenuButton is not None and self.currentMenuButton is menuButton:
            self.currentMenuButton = None
            self.updateSensitiveUpDownButtons()
            return
        ## toggle false  old current button
        if self.currentMenuButton is not None and self.currentMenuButton is not menuButton:
    ...
    def searchEntryOnChange(self, searchEntry):
        ...
        self.filteredActiveApps = self.filterHelper.filteredAppsByName(self.activeApps, self.showOnPanel, text)
    ...
    def updateSensitiveUpDownButtons(self):
        if self.currentMenuButton is not None:
            currentMenuButtonIndex = self.menuButtons.index(self.currentMenuButton)
    ...
    def updateAllAppsIndexes(self):
        ...
        #     print("%s, %s" %(app.getName(), app.getIndex()))
    ####################################
    # update END
    ####################################
    ...
    def loadAndExtractApps(self, appsDict, treeRoot=None):
        ...
        if self.tree is None:
            ...
            self.tree = GMenu.Tree.new(self.APPS_ID, GMenu.TreeFlags.SORT_DISPLAY_NAME)
